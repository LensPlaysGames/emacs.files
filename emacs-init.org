...
On Windows, AutoHotkey seems like the best solution.
...
On my desktop, I use a keyboard (technically two that work in tandem)
loaded with the QMK firmware along with VIA support, so I have whatever
button do whatever I want it to do.
...
***** Setup buffer-local revert hook

#+begin_src emacs-lisp :mkdirp yes :tangle .emacs
    (defun apply-colors-on-revert-hook ()
      (ansi-color-apply-on-region (point-min) (point-max) 'preserve)
      (auto-revert-mode 1))

    (defun apply-colors-on-revert ()
      (interactive)
      (auto-revert-mode 1)
      (add-hook 'after-revert-hook
                #'apply-colors-on-revert-hook
                nil
                'local))

  (defun apply-colors-on-revert-stop! ()
      (interactive)
      (auto-revert-mode -1)
        ...
        (progn
          ...
          (unless (seq-contains wrap-at-column-whitespace (char-before))
            (if word
                (backward-word)
              (backward-sexp)))
          ;; Take into account "'" in the middle of words.
          (when (eq ?\' (char-before))
...
***** Handle =:<line>:<column>= When Opening Files

#+begin_src emacs-lisp
  (defun find-file-noselect--advice-around-with-line-number
      (orig-fun filename &rest args)
    "Handle trailing `:line:column` when opening files.
  Written as an advice around 'find-file-noselect'."
    (save-match-data
      (let* ((matched (string-match "^\\(.*\\):\\([0-9]+\\):\\([0-9]+\\):?$"
                                    filename))
             (line-number (and matched
                               (match-string 2 filename)
                               (string-to-number (match-string 2 filename))))
             (column-number (and matched
        ...
        (when (or line-number column-number)
        ...
        ;; Return the buffer for other things to use
        buf)))
  (advice-add #'find-file-noselect
              :around #'find-file-noselect--advice-around-with-line-number)
#+end_src
...
**** Set Custom Custom File

#+begin_src emacs-lisp :mkdirp yes :tangle .emacs
  ;; Set Custom Custom File
  (setf custom-file (expand-file-name "my-custom.el" user-emacs-directory))
  (load custom-file)
#+end_src
...
#+begin_src emacs-lisp :mkdirp yes :tangle .emacs
  ...
  (defun lens/eglot-ensure ()
    "Like 'eglot-ensure', but actually ensures project root is findable,
    relevant packages loaded, etc."
    (require 'project)
    (require 'magit)
    (eglot-ensure))
  (use-package eglot
    ...
    (add-to-list 'eglot-server-programs
                 `((c-mode c++-mode objc-mode cuda-mode)
                   . ("clangd"
                      ...
                      ,(format "-j=%s" (min 8 (1- (num-processors)))))))
    :custom
...
#+begin_src emacs-lisp :mkdirp yes :tangle .emacs
  ...
  ;; NOTE: I prefer tmr-with-details
  (use-package tmr
    :defer t)
#+end_src
...
[[https://ledger-cli.org][Ledger, the CLI Finance Tool]]

#+begin_src emacs-lisp :mkdirp yes :tangle .emacs
  ;;; Ledger mode -- The CLI Finance Tool
  ;; https://github.com/ledger/ledger-mode
  (use-package ledger-mode
    :ensure t
    :custom (ledger-highlight-xact-under-point nil))
#+end_src

**** Writing

***** Screenplays via Fountain Mode
...
I don't like LLVM, but I do like well-formatted and consistent code
within a project, soooo...

#+begin_src emacs-lisp :mkdirp yes :tangle .emacs
  ;;; clang-format
  (use-package clang-format)

  (defun clang-format-buffer-before-save-hook ()
    "Add a buffer local hook to 'before-save-hook' that runs clang-format
  on the current buffer iff there is a \".clang-format\" file located"
    (add-hook
     'before-save-hook
     ;; Lambda that clang-formats the current buffer iff there is a .clang-
     ;; format file present in the current or parent directories.
     (lambda ()
       (when (locate-dominating-file "." ".clang-format")
         (clang-format-buffer))
       nil)
     ;; Make hook buffer local
     nil t))

  (add-hook 'c-mode-common-hook 'clang-format-buffer-before-save-hook)
  (add-hook 'c-ts-mode-hook 'clang-format-buffer-before-save-hook)
  (add-hook 'c++-ts-mode-hook 'clang-format-buffer-before-save-hook)
#+end_src
...
#+begin_src emacs-lisp :mkdirp yes :tangle .emacs
  ;;; titlecase
  ;; It turns out, capitalizing titles is hard.
...
#+begin_src emacs-lisp :mkdirp yes :tangle .emacs
  ;;; htmlize
  ;; Convert your current buffer into HTML, stylized to keep any decorations
  ;; applied.
...
#+begin_src emacs-lisp :mkdirp yes :tangle .emacs
  ...
  ;; NOTE: Severely messes with 'project.el', so we must add a function to
  ;; 'project-find-functions'.
  (setq vc-handled-backends nil)
  (defun lens/find-project-root--magit (dir)
    "For use as a hook within 'project-find-functions'."
    (when (featurep 'magit)
...
#+begin_src emacs-lisp :mkdirp yes :tangle .emacs
  ...
  ;; Compilation Mode ANSI sequences
...
**** Disable Exit Footgun

It's just too easy... If you wan't out, do =M-x sav-e C-RET=
which is really =M-x save-buffers-kill-emacs C-RET=.

#+begin_src emacs-lisp :mkdirp yes :tangle .emacs
  ;; It's just too easy... If you wan't out, do `M-x sav-e C-RET`
  ;; which is really `M-x save-buffers-kill-emacs C-RET`.
  (keymap-unset (current-global-map) "C-z" t)
  (keymap-unset (current-global-map) "C-x C-z" t)
#+end_src
