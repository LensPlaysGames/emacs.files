...
TODO: It would be cool if lens/wrap-comment didn't just use 72
hardcoded, but 72 from the beginning of the text on the line...
...
Update: It is likely that this build of Emacs doesn't have the "play"
directory included for some reason. Things like ~tetris~, ~life~, etc
are all missing. So this config is fine, it's just my Emacs binaries
that are messed up.
