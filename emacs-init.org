...
#+begin_src emacs-lisp :mkdirp yes :tangle .emacs
  ...
  (defun wrap-at-column (column &optional prefix word original-point)
    "Wrap current line into multiple that are COLUMN length or under.
      ...
      ORIGINAL-POINT is used internally to restore point."
  ...
  (defun lens/wrap-comment ()
    ...
    (let ((original-point (point)))
      ...
      (let ((prefix (buffer-substring (line-beginning-position) (point))))
        (wrap-at-column 72 prefix nil original-point))))
#+end_src
...
***** Emacs Command Logging

This plugin seems like it would be really cool, but it is written in a
horribly unusable fasion. Not customizable. Hard-coded window
parameters and having a buffer name that starts with space and
therefore isn't listed anywhere buffers are is just plain *bad*.

#+begin_src emacs-lisp :mkdirp yes :tangle no
  (use-package command-log-mode
    :defer t
    :config
    ;; Display command log buffer in a thin side window on the very right.
    (add-to-list
     ...
     `(" ?\\*command-log\\*"
       ...
       (slot . 1)
       (window-parameters
        ;; Do not allow switching to the command log buffer.
        (no-other-window . t)
        (mode-line-format . ("" " %b")))))
    (with-eval-after-load 'org
      (push 'org-beginning-of-line clm/log-command-exceptions*)
      (push 'org-end-of-line clm/log-command-exceptions*)
      (push 'org-self-insert-command clm/log-command-exceptions*)
      (push 'org-delete-char clm/log-command-exceptions*)
      (push 'org-delete-backward-char clm/log-command-exceptions*))
    (with-eval-after-load 'isearch
      (push 'isearch-printing-char clm/log-command-exceptions*)))
#+end_src
